...
```{r}
Moracle = est.beta( Y.1 + Y.0 ~ Z, ~ A + B, data=df )
Moracle
SE( Moracle )
```
...
est.beta( Y.1 + Y.0 ~ Z, ~ A + B, data=df )$beta.hat
...
```{r}
rs = est.beta( Yobs ~ D | Z, ~ A + B, data=data )
rs
rs$beta.hat
SE( rs )
...
## The 2SLS Approach
```{r}
rs = est.beta( Yobs ~ Z | D,  ~ A + B, data=data, method="2SLS" )
rs
SE( rs )
rs$beta.hat
rs$beta.hat - beta
